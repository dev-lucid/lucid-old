...
$grid = new lucid_html_grid(
    ...
    [
        ...
        new lucid_html_grid_column('Organization','organization_name',true,'20%',function($column,$format,$data){
            return '<a href="#!account/edit_organization|org_id|'.$data->org_id.'">'.$data->organization_name.'</a>';
        }),
        ...
        new lucid_html_grid_column('Name','first_name',true,'30%',function($column,$format,$data){
            return '<a href="#!account/edit_user|user_id|'.$data->user_id.'">'.$data->first_name.' '.$data->last_name.'</a>';
        }),
        new lucid_html_grid_column('Dates','last_login',true,'20%',function($column,$format,$data){
            $html = 'Last Login: '.(($data['last_login'] == '')?'Never':lucid_format::date($data['last_login'],'US-date')).'<br />'.'Created On: '.lucid_format::date($data['created_on'],'US-datetime');
            return $html;  
