...
lucid.handleHashchange=function(){
    ...
    */
    var urlParts = lucid.parseHashUrl(window.location.hash);

    lucid.request(urlParts.url,urlParts.data,true);
};

lucid.deleteConfirm=function(deleteUrl){
    if(confirm('Are you sure you want to delete this item?')){
        var request = lucid.parseHashUrl(deleteUrl);
        lucid.request(request.url,request.data);
    }
};

lucid.parseHashUrl=function(hashUrl){
    var urlData  = {};
    var urlParts = (new String(hashUrl).replace('#!','')).split('|');
    console.log(urlParts);
    newUrl = urlParts[0];
    for(var i=1;i<urlParts.length;i+=2){
        urlData[urlParts[i]] = decodeURIComponent(urlParts[i + 1]);
    }
    return {'url':newUrl,'data':urlData};
};
